...
SkipList buildlist(int num) {
	...
	if (input!=NULL) {
		u=fscanf(input, "%u", &level);
		d = skiplist_create(level);
		
		u=fscanf(input, "%u", &nb_values);
		for (unsigned int i=0;i< nb_values; ++i) {
			u=fscanf(input, "%d", &value);
...
void test_search(int num){
    ...
    int nb_found = 0, n, value;
    SkipList ltest = buildlist(num);
    char *nom_fichier = gettestfilename("search",num);
    ...
    unsigned int sum = 0;
    for(int i = 0 ; i < n ; i++){
        if (fscanf(fichier, "%d", &value) != 1){
            perror("Erreur lecture du fichier");
            exit(1);
        }
        if(skiplist_search(ltest, value, &nb_operations)){
            printf("%d -> true\n",value);
            nb_found++;
        }
        else
